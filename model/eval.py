...
def batch_level_evaluation(model, pooler, device, method, dataset, n_dim=0, e_dim=0):
    ...
    graphs = data['dataset']
    train_malicious = data['malicious_index']
    # Filter out indices that are in train_malicious, preserving the original order
    test_indices = [i for i in full if i not in train_malicious]
    with torch.no_grad():
    ...
    y = np.array(y_list)
    if 'knn' in method:
        test_auc, test_std = evaluate_batch_level_using_knn(300, dataset, x, y)
    else:
...
def evaluate_entity_level_using_knn(dataset, x_train, x_test, y_test):
    ...
    pred_test = np.array([])  # Initialize as an empty array
    if dataset == 'cadets':
        n_neighbors = 200
    else:
        n_neighbors = 10
    ...
    save_dict_path = './distance_result/distance_save_{}.pkl'.format(dataset)
    ...
    for i in range(len(f1)):
        ...
        if dataset == 'trace' and rec[i] < 0.996:
            best_idx = i - 1
            break
        if dataset == 'theia' and rec[i] < 0.99995:
            best_idx = i - 1
            break
        if dataset == 'cadets' and rec[i] < 0.99:
            best_idx = i - 1
            break
    best_thres = threshold[best_idx]
